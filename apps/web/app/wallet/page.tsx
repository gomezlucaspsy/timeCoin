...
export default function WalletPage() {
  ...
  function handleImport(e: React.FormEvent) {
    ...
    try {
      ...
      setMineMessage("Clave privada inválida.");
    }
  }
  ...
  return (
    <div className="flex flex-1 flex-col bg-zinc-50 font-sans dark:bg-black">
      ...
      <main className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-6 px-6 py-10">
        ...
        <section className="rounded-2xl border border-black/[.08] bg-white p-6 dark:border-white/[.08] dark:bg-zinc-950">
          <h2 className="mb-3 font-medium">Clave privada</h2>
          <p className="mb-3 text-sm text-zinc-600 dark:text-zinc-400">
            Se guarda solo en este navegador (localStorage). Si la perdés, perdés el acceso al saldo.
          </p>
          <button
          ...
          {showPrivateKey && wallet && (
            <p className="break-all rounded-lg bg-zinc-100 p-3 font-mono text-xs dark:bg-zinc-900">
              {wallet.privateKeyHex}
            </p>
          )}

          <form onSubmit={handleImport} className="mt-4 flex flex-col gap-3">
            <label className="text-sm text-zinc-600 dark:text-zinc-400">Importar otra wallet (clave privada en hex)</label>
            <input
              type="text"
              value={importKey}
